...
public class ScrollBehavior {
	...
	/**
	 * Sets the global boolean and changes the elevation manually if necessary.
	 * If scrollY of the scrollView is 0, overScroll is turned off.
	 * Otherwise it's on if the view is scrollable.
	 */
		...
		scrollView.getViewTreeObserver().addOnGlobalLayoutListener(
				new ViewTreeObserver.OnGlobalLayoutListener() {
					...
					public void onGlobalLayout() {
						...
						} else if (DEBUG) Log.e(TAG, "measureScrollView: no child");
	...
	private void tintAppBarLayout(@ColorRes int target) {
		...
		if (appBarColor == targetColor) return;
